      ...
      <div style={{ width:'100%', maxWidth:430, background:'white', minHeight:'100vh', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ padding:'16px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'1px solid #f3f4f6', position:'sticky', top:0, background:'white', zIndex:10 }}>
          ...
          <div style={{ position:'relative' }}>
            <button onClick={() => setMenuOpen(o => !o)} style={{ width:36, height:36, borderRadius:'50%', background:'linear-gradient(135deg,#7C3AED,#EC4899)', border:'none', cursor:'pointer', color:'white', fontWeight:800, fontSize:15 }}>
              {initial}
            </button>
            {menuOpen && (
        ...
        <div style={{ padding:'24px 20px', flex:1 }}>
          <div style={{ marginBottom:24 }}>
            ...
            <p style={{ color:'#6b7280', fontSize:14, marginTop:4 }}>
              {user?.class} • {user?.branch} • Division {user?.division}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign:'center', padding:40, color:'#9ca3af' }}>
              ...
              <p>No subjects found for your class/branch.</p>
            </div>
          ) : (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {subjects.map((s, i) => (
                <button key={s.id} onClick={() => nav(`/student/units/${s.id}`)}
