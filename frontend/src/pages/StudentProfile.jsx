...
export default function StudentProfile() {
  ...
  const save = async () => {
    ...
    try {
      const body = { email: form.email, division: form.division, class_year: form.class_year, roll_number: form.roll_number }
      ...
      login({ ...user, email: form.email, division: form.division, class_year: form.class_year, roll_number: form.roll_number }, token)
    ...
    <div style={{ minHeight:'100vh', background:'#f3f4f6', display:'flex', justifyContent:'center' }}>
      <div style={{ width:'100%', maxWidth:430, background:'white', minHeight:'100vh', display:'flex', flexDirection:'column' }}>
        ...
        <div style={{ padding:'24px 20px', flex:1 }}>
          <button onClick={() => nav('/student/subjects')} style={{ background:'none', border:'none', cursor:'pointer', color:'#7C3AED', fontSize:14, fontWeight:600, display:'flex', alignItems:'center', gap:4, marginBottom:20, padding:0 }}>
            ← Back
          </button>

          <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:28 }}>
            <div style={{ width:56, height:56, borderRadius:'50%', background:'linear-gradient(135deg,#7C3AED,#EC4899)', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:800, fontSize:22, fontFamily:'Nunito,sans-serif', flexShrink:0 }}>
              {initial}
            </div>
            <div>
              <h2 style={{ fontFamily:'Nunito,sans-serif', fontWeight:800, fontSize:20, margin:0 }}>{user?.username}</h2>
              <p style={{ fontSize:13, color:'#9ca3af', margin:'2px 0 0' }}>Student • {user?.branch}</p>
            </div>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              <div>
                ...
                <select value={form.class_year} onChange={e=>set('class_year',e.target.value)} style={{...inp, background:'white'}}>
                  {CLASSES.map(c=><option key={c}>{c}</option>)}
                </select>
              </div>
              <div>
                <label style={lbl}>Division</label>
                <select value={form.division} onChange={e=>set('division',e.target.value)} style={{...inp, background:'white'}}>
            ...
            <div>
              ...
              <input value={form.roll_number} onChange={e=>set('roll_number',e.target.value)} style={inp} placeholder="e.g. 101"/>
            </div>
